...
import 'package:architect/features/architect/presentations/page/skeleton/team_create.dart';
...
class TeamPageCreate extends StatefulWidget {
  const TeamPageCreate({Key? key, required this.user}) : super(key: key);

  final User user;

  @override
  State<TeamPageCreate> createState() => _TeamPageCreateState();
}

class _TeamPageCreateState extends State<TeamPageCreate> {
  ...
  void onSubmit() {
    ...
    teamBloc.stream.listen((event) {
      if (event.statusTeam == TeamStatus.loading) {
      ...
      } else if (event.statusTeam == TeamStatus.failure) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      } else if (event.statusTeam == TeamStatus.success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: ErrorDisplay(message: 'Team created!'),
          ),
        );
        ...
        child: BlocBuilder<UserBloc, UserState>(
          builder: (context, state) {
            if (state is UserInitial) {
              return const Center(child: TeamCreateShimmer());
              ...
              return Scaffold(
                ...
                body: RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: const [
                      ErrorDisplay(message: 'Connect to internet. Refresh it.'),
                    ],
                  ),
                ),
              );
                    ...
                    decoration: BoxDecoration(
                      ...
                      image: DecorationImage(
                        image: imagePath.isNotEmpty
                            ? FileImage(File(imagePath.isEmpty
                                ? widget.user.image
                                : imagePath))
